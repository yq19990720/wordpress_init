...
class EbayApi
{
    ...
    final public function productCategoryList(string $update = null): array
    {
        ...
        //缓存列表
        $data = get_mysql_options('banner', BANNER_MYSQL_TIME);
        //过期时间一天
        ...
        if ($res->status == 200) {
            foreach ($res->data->items as $value) {
                $price = get_rate_price(
                    $value->currency_id,
                    determine_locale_currency(),
                    floor($value->current_price * 0.5 * 100) / 100
                );
    ...
    //获取产品详情
    final public function productDetails(string $item_id, string $recommend, string $type = null): array
    {
        global $jsonld;
        if (!$type) {
        ...
        if ($res->status == 200) {
            $price = get_rate_price(
                $data->currency_id,
                determine_locale_currency(),
                floor($data->current_price * 0.5 * 100) / 100
            );
            $ProductDetails[] = (object)array(
                ...
                'post_url'           => Urlcode(
                    ...
                    . "_" . Page_code("sku" . $data->item_id, "en") . ".html",
                'post_type'          => "product",
                "usd_price"         => get_rate_price(
                    $data->currency_id,
                    determine_locale_currency(),
                    floor($data->current_price * 0.5 * 100) / 100,
                    'USD'
    ...
    //get exchange rate
    final public function getRate(): array
    {
        $rates = get_mysql_options('rate', RATE_MYSQL_TIME);
        if (!$rates || $rates->request) {
        //汇率不存在，或者返回信息中携带请求对象则再次请求汇率接口
            //获取接口汇率缓存
    ...
    //router
    final public static function router(): array
    {
        $post_sign = $_GET['sign'];
...
function get_currency_price($currency_code = '', $payment_way = '', $price = '')
{
    ...
    //货币代码
    $code = CURRENCY_CODE[$currency_code]['code'];
//define定义的常量用key=>value的取法只适用版本7+，7一下的不支持需赋值给变量后取出
    //校验货币代码是否存在
...
function get_paypal_payment($payment, $currency_code, $price)
{
     ...
     * 2、是否支持该货币付款
     */
    $is_decimal = $payment['is_decimal'];
//是否支持小数点支付
    $is_cur_not_sup = $payment['is_cur_not_sup'];
//是否支持该货币付款
    //不支持则返回false
    if (!$is_cur_not_sup) {
        throw new Exception(
            sprintf(
                __("This payment method does not support " . $currency_code . " payment in this currency")
            )
        );
    }
    //不支持小数->四舍五入
...
function get_stripe_payment($payment, $code, $price, $currency_code)
{
     ...
     * 2、付款最低限制，无则是false
     */
    $is_zero_decimal = $payment['is_zero_decimal'];
//是否是零位十进制
...
function get_mysql_options($like, $expire)
{
    ...
    $get_data = $wpdb->get_results(
        "SELECT option_value,autoload FROM "
        . $wpdb->options .
        " WHERE 1=1 AND option_name LIKE '$like'"
    );
...
function get_description()
{
    ...
    if ($url == 'shop') {
        ...
        $description =  $currency_list_symbol
            . $price . ',' . $title . ',' . $Category_name . ',' . $title_three . ',' . LIST_PAGE_META_2;
    } elseif ($url == 'detail') {
        $description =  CURRENCY_CODE[$ebay_product_detail[0]->currency_id]['code']
            . $ebay_product_detail[0]->description_price . ','
            . substr(
                htmlspecialchars(
                    preg_replace(
                        '/<\/?[^>]+>/',
                        ...
                        htmlspecialchars_decode(
                            $ebay_product_detail[0]->post_content
                        )
                    )
                ),
                0,
...
//查询该条数据是否存在，存在更新不存在插入
    $get_data = $wpdb->get_results(
        "SELECT option_value,autoload FROM " . $wpdb->options . "
         WHERE 1=1 AND option_name LIKE '$like'"
    );
    $data_value = json_decode($get_data[0]->option_value);
    $data_value->response = true;
//如果接口未正常响应则添加response属性设置为true
    ...
    //update
        ...
        $data ? $wpdb->query(
            "UPDATE " . $wpdb->options .
            " SET option_value='{$options_value}',
            autoload='{$time}' WHERE option_name = '{$like}'"
        ) :
            $wpdb->query(
                "UPDATE " . $wpdb->options .
                " SET option_value='{$data_value}',
                 autoload='{$time}' WHERE option_name = '{$like}'"
            );
    } else {
        $wpdb->insert(
            "wp_options",
            array("option_name" => $like,
