...
//		add_action( 'after_switch_theme', array( $this, 'switch_theme' ) );
	...
	function required_classes() {
		if ( ! class_exists( '\WP_Importer' ) ) {
			require ABSPATH . '/shopify_seo/includes/class-wp-importer.php';
		}
	...
	public function admin_page() {
		...
		// Enqueue styles.
		wp_enqueue_style( 'merlin', trailingslashit( $this->base_url ) . $this->directory . '/assets/css/merlin' . $suffix . '.css', array( 'shopify_seo' ), MERLIN_VERSION );
	...
	public function svg( $args = array() ) {
		...
		if ( empty( $args ) ) {
			return __( 'Please define default parameters in the form of an array.', 'woocommerce' );
		}

		// Define an icon.
		if ( false === array_key_exists( 'icon', $args ) ) {
			return __( 'Please define an SVG icon filename.', 'woocommerce' );
		}
					...
					<?php foreach ( $required_plugins as $slug => $plugin ) : ?>
						<li data-slug="<?php echo esc_attr( $slug ); ?>">
							...
							<label for="default_plugins_<?php echo esc_attr( $slug ); ?>">
								...
								<span class="badge">
									<span class="hint--top" aria-label="<?php esc_html_e( 'Required', 'woocommerce' ); ?>">
										<?php esc_html_e( 'req', 'woocommerce' ); ?>
									</span>
								</span>
							</label>
						</li>
			...
			<?php if ( 1 < count( $this->import_files ) ) : ?>
				...
				<div class="merlin__select-control-wrapper">
					...
					<div class="merlin__select-control-help">
						<span class="hint--top" aria-label="<?php echo esc_attr__( 'Select Demo', 'woocommerce' ); ?>">
	...
	protected function edd_activate_license( $license ) {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				$message = esc_html__( 'An error occurred, please try again.', 'woocommerce' );
			}
		} else {
			...
			if ( false === $license_data->success ) {
				...
				switch ( $license_data->error ) {
					...
					case 'revoked':
						$message = esc_html__( 'Your license key has been disabled.', 'woocommerce' );
						break;

					case 'missing':
						$message = esc_html__( 'This appears to be an invalid license key. Please try again or contact support.', 'woocommerce' );
						break;

					case 'invalid':
					case 'site_inactive':
						$message = esc_html__( 'Your license is not active for this URL.', 'woocommerce' );
						break;
					...
					case 'no_activations_left':
						$message = esc_html__( 'Your license key has reached its activation limit.', 'woocommerce' );
						break;
	...
	function _ajax_plugins() {
		...
		if ( $json ) {
			...
			$json['message'] = esc_html__( 'Installing', 'woocommerce' );
			wp_send_json( $json );
		} else {
			$this->logger->debug(
	...
	public function pt_importer_new_ajax_request_response_data( $data ) {
		...
		$data['message']  = esc_html__( 'Installing', 'woocommerce' );
		$data['proceed']  = 'true';
