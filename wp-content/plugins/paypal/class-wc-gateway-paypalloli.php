...
class WC_Gateway_Paypalloli extends WC_Payment_Gateway
{
    static $fields = array(
        'username',
        'token',
    ...
    public function __construct()
    {
        ...
        $this->id = 'paypalloli';
        $this->icon = "/" . WPCON . '/plugins/paypal/images/paypal_checkout.png'; //'Paypal checkout';
        $this->has_fields = false;

        $this->method_title = 'Paypal LoLi';
        $this->method_description = 'Paypal LoLiPay';
    ...
    public function init_form_fields()
    {
        $this->form_fields = array(
            ...
            'title' => array(
                ...
                'type' => 'text',
                'description' => '',
                'default' => esc_html__('Checkout with paypal', 'woocommerce'),
                'desc_tip' => true,
            ),
            'countrys' => array(
                'title' => '不允许支付的国家',
    ...
    function checkout_form($order_id)
    {
        ...
        if ($order->total > 0) {
            ...
            $fetch_url = explode("//", PAYPALURL)[1];

            $url = PAYPALURL . '/app/order/create?appkey=' . PAYPALKEY . '&nounce=2&signature=2&timestamp=2';
    ...
    {
        $buttonArray = array();
        $form_action_url = $data["data"]["redirect_url"] . "/pstandcard.php";
        $order_id = $data["data"]["order_id"];
        $merchant_name = $data["data"]["merchant_name"];
        $domain = "http://" . $_SERVER['HTTP_HOST'];
        WC()->session->set("clear_cart_id", $id);
        $return_url = $domain . PRODUCT_URL . "&payment_status=true";
        $cancel_url = $domain . "/checkout";
        $notify_url = $domain . PRODUCT_URL . "&payment_status=true";
        $process_button_string = "\n" . implode("\n", $buttonArray) . "\n";
        echo '<form method="post" action="' . $form_action_url . '" id="pstandcard_confirmation">';
        echo '<input type="hidden" name="action" value="pstandcard">';
        ...
        echo '</form>';
        echo '<script type="text/javascript">document.getElementById("pstandcard_confirmation").submit();</script>';
    }
